<template>
    <el-container class="home-container">
      ...
      <el-container>
        ...
        <el-main class="el-main">
            <el-button type="info" class="buttonper" @click="yearHot()">年度热品</el-button>
            <el-button type="info" class="buttonper" @click="monthHot()">月度热品</el-button>
            <el-button type="info" class="buttonper" @click="weekHot()">周度热品</el-button>
            ...
            <el-table :data="this.list" style="width: 100% margin:auto">
                ...
                <el-table-column prop="collectNum" label="阶段收藏量" />
                <el-table-column label="操作" v-slot:default="scope" width="350">
                    <el-button type="info" class="buttonper" @click="checkdetail(scope.row.concreteProductID)">详细信息
                        </el-button>
                    <el-button type="info" class="buttonper" @click="choosefavorite(scope.row.concreteProductID)">收藏
                        </el-button>
                    <el-button type="info" class="buttonper" @click="choosehotsearch(scope.row.concreteProductID)">实时热度
                        </el-button>
                    </el-table-column>
            </el-table>
            <div>
    ...
    <el-dialog v-model="this.favoritedialog" title="设置价格下限" width="80%" >
            ...
            <el-dialog v-model="this.hotsearchdialog" title="实时热度" width="80%" height="80%" >
                <div>
        <div id="myChartRelation" style="width: 100%; height: 400px; margin:0 auto; margin-top: 5%;"></div>
    <!-- <el-table :data="this.pricehistoryweekReverse" style="width: 100%" >
		<el-table-column type="index" label="#" width="100" align="center" header-align="center">
			</el-table-column>
                <el-table-column prop="price" label="价格"  align="center" 
    ...
    export default{
        ...
        methods:{
            ...
            loadData(){
                ...
                // formData.append('dateRange', 'year');
                
                // this.$axios.post('/collect/order',formData)          //这里填后端端口
                // .then(res=>{
            ...
            monthHot(){
                this.dateRange=1
                var formData = new FormData();
                formData.append('dateRange', 'month');
                this.$axios.post('/collect/order',formData)          //这里填后端端口
                .then(res=>{
                    this.data=res.data.contents
                    console.log(this.data)
            ...
            weekHot(){
                this.dateRange=2
                var formData = new FormData();
                formData.append('dateRange', 'week');
                this.$axios.post('/collect/order',formData)          //这里填后端端口
                .then(res=>{
                    this.data=res.data.contents
                    console.log(this.data)
            ...
            checkPriceHistoryYear(){
                ...
                .then(res=>{
                    ...
                    console.log(this.pricehistoryyearTime)
            ...
            choosehotsearch(id){
                this.hotsearchdialog = true;
                var formData = new FormData();
                formData.append('concreteProductID', id);
                this.$axios.post("/collect/increse",formData)          //这里填后端端口
                .then(res=>{
                    console.log(res.data.contents)
                    this.hotinfoCollect=res.data.contents
                    this.hotinfoCollectNum=this.hotinfoCollect.map(item=>item.increasement)
                    this.hotinfoDate=this.hotinfoCollect.map(item=>item.date)
                

                var formData1 = new FormData();
                formData1.append('concreteProductID', id);
                this.$axios.post("/product/pricechange",formData1)          //这里填后端端口
                .then(res=>{
                    console.log(res.data.contents)
                    this.hotinfoPrice=res.data.contents
                    this.hotinfoPriceNum=this.hotinfoPrice.map(item=>item.price)
                   
                 // 在对话框打开后，初始化图表实例并绑定到 $refs.chartContainer
                this.myChartRelation = echarts.init(document.getElementById('myChartRelation'), 'light');
                    const options = {
                        legend: {	//图表上方的图例
                            data: ['近一月商品价格','近一月新增收藏']
                        },
                        xAxis: {
                            type: 'category',
                            name: '日期',   // x轴名称
                        ...
                        series: [
                            {
                                name: '近一月商品价格',
                                type: 'line',
                                data: this.hotinfoPriceNum
                            },
                            {
                                name: '近一月新增收藏',
                                type: 'line',
                                data: this.hotinfoCollectNum
                            }
                        ]
                    };

                    this.myChartRelation.setOption(options);
                })
            })
                setTimeout(() => {
                // 在延迟后打开对话框的代码
